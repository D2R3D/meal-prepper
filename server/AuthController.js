...
const register = async (req, res) => {
    ...
    const {username, email, password, favorite_foods, profile_pic} = req.body
    ...
    const newUser = await db.register_user([username, email, hashPass, favorite_foods, profile_pic])
...
const login = async (req, res) => {
    ...
    if(authedPassword) {
        ...
        res.status(401).send('Username or Password incorrect, please try again.').catch(err => console.log(err))
    }
};
